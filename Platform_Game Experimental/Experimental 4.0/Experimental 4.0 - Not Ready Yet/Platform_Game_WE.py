...
F.write("\n")
F.write("import Options as OPS")
F.write("\n")
F.write("import Classes as C")
F.write("\n")
...
F.write("   Power_Up = {}")
F.write("\n")
F.write("   Trump_Wall = L.level(0, 0-L.Screen_DiffX, 0-L.Screen_DiffY, 40, 1680, black, False)")
F.write("\n")
F.write("   Origin_Block = L.level(0, 1-L.Screen_DiffX, 0-L.Screen_DiffY, 1, 1, white, True)")
F.write("\n")
F.write("   E1 = L.level(0,OPS.width/2, 948-L.Screen_DiffY, 100, 20, black, True)")
F.write("\n")
...
def Custom():
    ...
    if not(x >= 1680):
        ...
        if pygame.mouse.get_pressed()[2] == True: #Left Click and drag to draw
            if PGW.Id == 1:
                ...
                if PGW.Image == False:
                    ...
                    F.write("   B[" + str(st) + "] = L.level("+ str(PrevX1)+ ", " + str(x+(1680*Page)) + "-L.Screen_DiffX" + ", " + str(y) + "-L.Screen_DiffY" + ", " + str(width)+ ", " + str(height) + ", [" + str(Tab.R) + ", " + str(Tab.G) + ", " + str(Tab.B) + "], " + str(True) + ")")
                    F.write("\n")
                elif PGW.Image == True:
                    B[st] = level(PrevX1, x, y, width, height, black, True, True, Tab.Pictures[Tab.Picture_ID])
                    F.write("   B[" + str(st) + "] = L.level("+ str(PrevX1)+ ", " + str(x+(1680*Page)) + "-L.Screen_DiffX" + ", " + str(y) + "-L.Screen_DiffY" + ", " + str(width)+ ", " + str(height) + ", " + str(black)+ ", " + str(True) + ", " + str(True) + ", " + "Pictures[" + str(Tab.Picture_ID) + "]" + ")")
                    F.write("\n")
        ...
        if pygame.mouse.get_pressed()[1] == True and PGW.Cant_Enemy == False:
            ...
            F.write("   Enemy[" + str(PGW.StEnemy) + "] = SC.Enemy(" + str(PGW.x1+(1680*Page)) + "-L.Screen_DiffX" + ", "+ str(PGW.y1) + "-L.Screen_DiffY" + ")")
            F.write("\n")
...
def PowerUP():
     if pygame.mouse.get_pressed()[0] == True and Tab.Choosen_PowerUp != "Null" and time.clock() - PGW.T1 > 0.25: #Left Click to place the power up
         PGW.x, PGW.y = pygame.mouse.get_pos()
         if PGW.x < 1680 and PGW.x != PGW.PrX:
             PGW.PrX = PGW.x
             PGW.T1= time.clock()
             PowerUps = {}
             F.write("   Power_Up[" + str(PGW.st) + "] = C.Power_Ups('" + Tab.Choosen_PowerUp +"', " + str(PGW.x+(1680*Page)) + ", " + str(PGW.y-32) +")")
             F.write("\n")
             PowerUps[PGW.st] = PGW.Power_Ups(Tab.Choosen_PowerUp, PGW.x, PGW.y)
             F.write("   PowerUpGroup.append(Power_Up[" + str(PGW.st) + "])")
             F.write("\n")
             PGW.PowerUpGroup.append(PowerUps[PGW.st])
             PGW.st += 1
...
while running == True:
    ...
    if PowerUpGroup != []:
        for i in range(0, len(PowerUpGroup)):
            PowerUpGroup[i].Display(surf_1)
    pygame.display.update()
    for event in pygame.event.get():
            if event.type == pygame.QUIT: #For ending the loop
                F.write("   return BlockGroup, EnemyGroup, PowerUpGroup")
                PGW.running = False
                F.close()
                pygame.quit()
                print("I closed")
